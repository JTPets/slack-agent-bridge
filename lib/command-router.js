...
async function handleScheduledTask(ctx = {}) {
    ...
    // LOGIC CHANGE 2026-09-16: the channel check is skipped for a handler that declares
    // it resolves its OWN destination. `check-inbox` posts into the agent it is handed,
    // so it needs one; `weekly-critique` resolves the agent whose schedule declares the
    // task, so demanding a channel from the CALLER's agent would refuse for a reason
    // that has nothing to do with whether the critique can run.
    ...
    try {
        ...
        // back to the caller's agent for a handler that does not say. Naming
        // ctx.agent.channel unconditionally was correct only while every deterministic
        // task posted where it was pointed; `weekly-critique` posts into the jester's
        // channel whichever channel the verb was typed in, and a verdict that named the
        // wrong one would be a confident false statement about where to go and look.
...
const COMMANDS = {
    ...
    // LOGIC CHANGE 2026-09-15: `create` completes the activation surface — the three
    // verbs above could turn an existing definition on and off, and nothing could make
    // one. It lives in lib/agent-create.js rather than lib/agent-activation.js because
    // the two have different durability (gitignored workspace state vs. a TRACKED file
    // the next pull discards), and one module would invite one sentence covering both.
    // It creates NO Slack channel and activates nothing: the new definition is
    // `planned`, with no schedule and no watches.
    create: {
    ...
    // LOGIC CHANGE 2026-09-16: `critique` — the jester's weekly post, on demand.
    //
    // It is registered here and NOT as a second route, which is the point: the cron
    // tick and this verb reach the same `getDeterministicTask('weekly-critique').run()`,
    // so a scheduled critique and an on-demand one cannot diverge. It was a
    // TASK_TEMPLATES entry until 2026-09-16, and the note above about templates not
    // being verbs still holds for the five that remain — a template is a dispatch and
    // a dispatch already has `/dispatch`. `weekly-critique` stopped being one.
    //
    // The guard in tests/command-router.test.js is what made this land in the same
    // change as the catalogue move rather than a commit later: a deterministic task
    // that is neither a registered verb nor an explicit NOT_COMMANDS entry fails the
    // suite, which is the enumerator doing exactly its job.
    critique: {
