 ...
 * active, a schedule, a resolved channel, a task name, and a task name that resolves
 * to either a deterministic handler or a template.
 *
 * LOGIC CHANGE 2026-09-15: it now checks `status: "planned"`, because the scheduler
 * now does too (WORK-TODO #3). A planned agent's job used to register and post into
 * a channel nothing polls — the third of three consequences arriving without the
 * other two. Registration follows the same declaration as joining and polling or it
 * does not happen.
 *
...
function describeSchedule(agent) {
    ...
    if (agent.status === 'planned') {
        return { registered: false, task, cron, kind: null, reason: 'schedule declared but the agent is not activated in this workspace' };
    }
    if (!agent.channel) {
        const declared = agent.channel_name ? `#${agent.channel_name} has not been resolved to a channel id` : 'the agent declares no channel';
        return { registered: false, task, cron, kind: null, reason: `schedule declared but ${declared}` };
    }
    if (!task) {
...
/**
 * ONE RULE. Which agents' channels this bridge acts on at all.
 *
 * LOGIC CHANGE 2026-09-15: joining and polling now derive from THIS single
 * function instead of being two rules maintained separately. They had drifted:
 * `joinableChannels` took every declared channel while `pollableChannels` took only
 * active agents', so story-bot — `planned`, with a real channel and a weekly
 * schedule — was joined to a channel the poll loop never read. Its job posted a
 * `TASK:` message every Friday that nothing executed. Three consequences of one
 * declaration, or none:
 *
 *   resolved  — the declared channel_name has an id in the local channel map
 *   joined    — the bridge is a member of it
 *   polled    — poll() reads it, so a TASK:/ASK: message there is executed
 *
 * An agent that is not active gets none of the three, which is what `planned` was
 * always supposed to mean. An agent whose channel_name has not resolved gets none
 * of them either, and that is REPORTED rather than skipped (lib/agent-scheduler.js).
 *
 ...
 */
function activeChannels(agents, bridgeChannel) {
    const seen = new Map();
    // The bridge channel comes from BRIDGE_CHANNEL_ID, not from the bridge agent's
    // own record — see docs/AGENTS.md. It is always acted on.
    if (bridgeChannel) seen.set(bridgeChannel, { channelId: bridgeChannel, agentId: 'bridge' });
    for (const agent of agents) {
        // The bridge agent is skipped BY ID, not by channel: BRIDGE_CHANNEL_ID and
        // the bridge record's own channel are separate facts that could disagree,
        // and adding the record's channel as a second entry would poll a channel
        // nobody asked for. This matches buildChannelsToPoll exactly.
        if (agent.id === 'bridge' || !agent.channel || seen.has(agent.channel)) continue;
...
/**
 * The channels the bridge POLLS. Same rule as joining, by construction.
 *
 * The live `buildChannelsToPoll()` lives at module scope in bridge-agent.js and
 * cannot be called from a test without loading the whole bridge, so the rule is
 * re-stated here, pure. `tests/agent-surface.test.js` pins the two in agreement by
 * reading the live function's source — a rule restated in two places with nothing
 * comparing them is the drift this file exists to catch.
 *
 * @param {object[]} agents - Registry records.
 * @param {string} bridgeChannel - config.BRIDGE_CHANNEL.
...
function pollableChannels(agents, bridgeChannel) {
    return new Set(activeChannels(agents, bridgeChannel).map(c => c.channelId));
}

/**
 * The channels the bridge JOINS at startup. Same rule as polling, same function.
 *
 * Joining a channel that already exists is safe and reversible; creating one is not,
 * and nothing here creates any.
 *
 * @param {object[]} agents - Registry records.
 * @param {string} bridgeChannel - config.BRIDGE_CHANNEL.
 * @returns {Array<{ channelId: string, agentId: string }>} Deduplicated by channelId.
...
function joinableChannels(agents, bridgeChannel) {
    return activeChannels(agents, bridgeChannel);
}
