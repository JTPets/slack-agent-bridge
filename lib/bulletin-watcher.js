...
async function processBulletin(slack, bulletin, options = {}) {
    ...
    for (const agent of watchingAgents) {
        ...
        try {
            ...
            // LOGIC CHANGE 2026-09-14: escalate. `result.errors` is returned to the
            // caller and the only production caller (bridge-agent.js, after posting a
            // task_completed bulletin) discards the whole result, so this was a dead
            // end. The observed instance is `not_in_channel` for story-bot: an agent
            // the scheduler arms but the join path deliberately excludes.
            await notifyOps(
                `:mailbox_with_no_mail: *Bulletin notification rejected* — agent \`${agent.id}\`, ` +
                `bulletin type \`${type}\`\n${postErr.message}\n` +
                `Channel: \`${agent.channel || 'none'}\`. \`not_in_channel\` means the bot is not a ` +
                'member — invite it, or remove the watch from agents.json.'
            ).catch(notifyErr => {
                console.error('[bulletin-watcher] Could not escalate notify failure:', notifyErr.message);
            });
        }
    }
