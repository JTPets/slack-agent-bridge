...
// LOGIC CHANGE 2026-09-15: the FORM's default turn budget is the CEILING, not the
// parser's default.
//
// The parser's DEFAULT_TURNS (50) is the right default for a message that carries no
// TURNS: line at all — it is a floor for prose someone typed by hand. The form is not
// that: the standing convention is `TURNS: 100` on every bridge dispatch
// (docs/EXECUTOR-CONTRACT.md section 7), and the form defaulting to 50 has already
// cost three runs half their budget.
//
// It is DEFINED AS MAX_TURNS rather than written as 100, which is what makes "the two
// cannot disagree" true rather than merely currently-true. The ceiling is enforced in
// exactly one place — the `parsed > MAX_TURNS` rejection in validateDispatchFields
// below, using the same imported constant — so a change to MAX_TURNS moves the
// default and the ceiling together, and a default above its own ceiling is not
// expressible. tests/dispatch-message.test.js asserts the identity, not the number.
const DISPATCH_DEFAULT_TURNS = MAX_TURNS;
...
function validateDispatchFields(input = {}, githubOrg = undefined) {
  ...
  if (!turnsRaw) {
    // LOGIC CHANGE 2026-09-15: a blank form field now means the ceiling, not the
    // parser's 50. See DISPATCH_DEFAULT_TURNS above.
    values.turns = DISPATCH_DEFAULT_TURNS;
