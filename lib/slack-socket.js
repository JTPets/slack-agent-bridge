 ...
 * explicitly, not relied on as a library default). What it does not do is say when
 * reconnection has stopped working — and a quietly dead socket is
 ...
 *   console), `downAlertMs`, `onSlashCommand`/`onInteractive` (THE COMMAND SEAM,
 *   both default null), `setTimeoutFn`/`clearTimeoutFn`.
...
async function startSocketMode(deps = {}) {
  const {
    ...
    onSlashCommand = null,
    onInteractive = null, // LOGIC CHANGE 2026-09-15: see THE COMMAND SEAM below.
  ...
  // ---- THE COMMAND SEAM -------------------------------------------------------
  // LOGIC CHANGE 2026-09-15: Attached to, and it has TWO halves — a slash command
  // opens a modal, and that modal comes back as an `interactive` envelope, so a
  // command that opens a form is inert without both. bridge-agent.js supplies
  // lib/dispatch-command.js's handlers; this module registers no command name. An
  // envelope nobody handles is still acknowledged, or the operator sees a timeout
  // with no trace here. Event names stay literal so the guard in
  // tests/slack-socket.test.js can enumerate this file's subscriptions and keep
  // message intake single-owned: `interactive` is NOT a message event.
  const seamHandler = (handler, describe, fallbackAck) =>
    guarded(async ({ ack, body }) => {
      if (typeof handler === 'function') return handler({ ack, body });
      const what = describe(body);
      logger.warn(`[slack-socket] Received ${what} but no handler is attached; acknowledging.`);
      try {
        await (fallbackAck === undefined ? ack() : ack(fallbackAck));
      } catch (err) {
        logger.error(`[slack-socket] Failed to acknowledge ${what}: ${err.message}`);
      }
    });

  client.on('slash_commands', seamHandler(onSlashCommand, (b) => (b && b.command) || 'unknown command',
    { response_type: 'ephemeral', text: 'That command is not wired up yet.' }));
  client.on('interactive', seamHandler(onInteractive, (b) => (b && b.type) || 'unknown interaction', undefined));
