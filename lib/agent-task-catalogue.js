...
const DETERMINISTIC_TASKS = {
    ...
    // LOGIC CHANGE 2026-09-16: `weekly-critique` MOVED here from TASK_TEMPLATES
    // (WORK-TODO #53, docs/JESTER-DESIGN.md). As a template it posted the prose
    // "Review the week's activities and provide contrarian takes" as a TASK: message,
    // with NO material attached: an agent's task cannot read Slack history (not even
    // its own channel) and its scratch clone is `--depth 1`, so the model was asked to
    // review a week it had never been shown. It would have written something that read
    // like a review. Same defect as `check-inbox` before 2026-09-14, same fix — code
    // gathers the facts, the model is given them.
    //
    // The handler still calls a model, which `check-inbox` does not. "Deterministic"
    // here means the MATERIAL is computed and the DESTINATION is decided by code; the
    // critique itself is a judgement and is the one part a model is for.
    'weekly-critique': {
        description: 'Weekly critique of the computed digest, in the jester\'s voice',
        run: async ({ slack, agent }) => {
            // Lazily required for the same reason as above: this module is loaded at
            // startup and by tests that want neither an LLM client nor a git subprocess.
            const { runWeeklyCritique } = require('./weekly-critique');
            return runWeeklyCritique({ slack, agent });
        },
    },
};
