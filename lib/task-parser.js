...
const { isValidRepo, isValidBranch, describeValue } = require('./git-identifiers');
...
function parseTask(text, githubOrg = DEFAULT_GITHUB_ORG) {
   ...
   * Returns the captured value, or null when the field is absent. A label present
   * only in a non-canonical form (lowercase, mixed case, or indented prose) is
   * recorded as an error so it is never silently ignored.
   */
  ...
  if (repoRaw !== null) {
    ...
    if (isValidRepo(repo)) {
      ...
      task.errors.push(
        `REPO: rejected ${describeValue(repoRaw.trim())} - expected owner/name using ` +
