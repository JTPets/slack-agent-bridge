...
function startScheduler(slack, options = {}) {
    ...
    for (const agent of agents) {
        ...
        // LOGIC CHANGE 2026-09-15: both of these were a silent `continue`, and they
        // fix opposite failures. Registering a PLANNED agent's job was the third
        // consequence of a declaration arriving without the other two — story-bot
        // posted a TASK: message every Friday into a channel the poll loop does not
        // read (WORK-TODO #3). Skipping a channel-less agent silently was the
        // reverse: jester, marketing and social-media each carry a schedule that has
        // never once fired and was never once mentioned at startup. A capability that
        // does not exist has to be said out loud, so both land in `refusals`, which is
        // posted to #sqtools-ops below.
        if (agent.status === 'planned') {
            refusals.push(`${agent.id}:${taskName || 'unnamed'} — the agent is not activated in this workspace`);
            continue;
        }
        if (!agent.channel) {
            const declared = agent.channel_name
                ? `#${agent.channel_name} has not been resolved to a channel id — create the channel, then activate the agent`
                : 'the agent declares no channel';
            refusals.push(`${agent.id}:${taskName || 'unnamed'} — ${declared}`);
            continue;
        }
