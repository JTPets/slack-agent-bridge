...
Post the analytics summary with actionable insights.`,
    ...
    // LOGIC CHANGE 2026-09-13: Dropped the hardcoded "(scheduled by email-monitor)"
    // suffix from this description. buildTaskMessage already appends
    // "(scheduled by <agentId>)" to every template, so this one template produced
    // "Check and triage email inbox (scheduled by email-monitor) (scheduled by
    // email-monitor)" in the Slack message - visible in production every 30 minutes.
    // The suffix belongs in buildTaskMessage, which is generic; the description is not.
    'check-inbox': {
