...
function runSmokeTest(options = {}) {
    ...
    // A timeout is a FAILURE, not a pass. spawnSync signals it via result.error
    // (code ETIMEDOUT) and a killed child; name it explicitly so ops sees
    // "timed out" rather than a truncated jest dump.
    if (result.error) {
        ...
        return {
            ...
            timedOut,
            error: timedOut
                ? `smoke test did not finish within ${timeoutMs}ms - treated as failure`
                : `could not run npm run test:smoke (${result.error.code || result.error.message})`,
        };
    }
    ...
    // status !== 0 with no result.error: real test failures. Keep the tail of the
    // output - the head is jest's config banner, the failing assertions are last.
    const output = (result.stderr || result.stdout || `npm run test:smoke exited ${result.status}`).trim();
    return { ok: false, error: output.slice(-800) };
}
