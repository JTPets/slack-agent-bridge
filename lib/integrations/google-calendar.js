...
// Returns start/end timestamps for tomorrow's date.
/**
...
// Allows the secretary to tell users about tomorrow's schedule.
/**
 * Fetch events for tomorrow from a specific calendar.
 *
 * @param {string} [calendarId='primary'] - Calendar ID to fetch events from
 * @returns {Promise<Array<{ title: string, start: string, end: string, status: string, recurring: boolean }>>}
 */
async function getTomorrowEvents(calendarId = 'primary') {
    try {
        ...
        console.error(`[google-calendar] Failed to fetch tomorrow's events for ${calendarId}:`, err.message);
        return [];
    }
}
...
/**
 * Fetch tomorrow's events from all configured calendars.
 *
 * @returns {Promise<Array<{ title: string, start: string, end: string, status: string, recurring: boolean, calendarId: string }>>}
 */
async function getAllTomorrowEvents() {
    const calendarIds = getCalendarIds();
    const allEvents = [];

    for (const calendarId of calendarIds) {
        const events = await getTomorrowEvents(calendarId);
        for (const event of events) {
            allEvents.push({ ...event, calendarId });
        }
    }
