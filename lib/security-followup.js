 ...
 * @returns {Promise<{tasks: Array, skipped: Array, errors: Array}>}
 */
async function processSecurityBulletin(slack, bulletin, options = {}) {
    const { includeMedium = false, dryRun = false } = options;
    ...
    for (const [file, findings] of fileGroups) {
        ...
        if (dryRun) {
            console.log(`[security-followup] DRY RUN - Would post to ${channelId}:\n${taskMessage}`);
            result.tasks.push({
                file,
                repo,
                agentId,
