...
function isBandwidthExhausted(exitCode, stdout, stderr) {
  // Must have exit code 1 (failure)
  if (exitCode !== 1) return false;

  // Output must be empty or very short (under MIN_REAL_OUTPUT_LENGTH chars)
  const trimmedOutput = (stdout || '').trim();
  if (trimmedOutput.length >= MIN_REAL_OUTPUT_LENGTH) return false;

  // stderr must contain bandwidth-related keywords
  if (!stderr) return false;
  return BANDWIDTH_EXHAUSTION_PATTERNS.some(pattern => pattern.test(stderr));
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        // LOGIC CHANGE 2026-03-27: Check for bandwidth exhaustion on exit code 1.
        // When output is empty/short and stderr contains bandwidth-related keywords,
        // this is likely a session/bandwidth limit, not a real task failure.
        // Throw BandwidthExhaustedError so caller can pause and retry.
        if (isBandwidthExhausted(code, stdout, stderr)) {
          reject(new BandwidthExhaustedError(
            `Claude CLI bandwidth exhausted. Queue paused. Will retry in 30 minutes.`
          ));
          return;
        }
