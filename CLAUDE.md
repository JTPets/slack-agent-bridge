...
│   ├── file-size-gate.js # THE rule engine for the 300-line-per-file limit: measure() walks every .js file from disk, evaluate() is pure and applies the rule, check() runs both against lib/validate-exceptions.json. FOUR ways to fail — an undeclared file over the limit, an entry whose file is gone or is back under the limit, an entry with a blank reason, a path declared twice. Run by `npm run validate`, guarded by tests/file-size-gate.test.js
...
│   ├── DISPATCH-AND-BATCH-MODEL.md # A PROPOSAL, nothing built: the operator's nightly batch cycle across the estate, as decided 2026-09-20. ONE dated integration branch per batch, cut from main, built by MERGING (never rebasing — a rebase keeps the `Closes` body and destroys the SHA that cites it), gated ONCE, reviewed and merged in the morning, then DELETED; the batch holds a LOCK on main and abandoning it is cheap because the source branches are durable; ONE code-change dispatch in flight per repo; work skewed toward what verifies by READING; a bisect capped at THREE drops. THE CONSTRAINT is the operator's review time. Distinct from AUTONOMOUS-LOOP-DESIGN.md, which it cites rather than restates: that is a machine loop in one repo with no human at the gate, this is a human-gated cycle across three. Carries the finding that the bridge cannot run two dispatches at once at all (bridge-agent.js:1867) and that "verified" means different things in the two repos
│   ├── COMMAND-SURFACE.md   # THE design of record for the command surface: why there are exactly TWO slash registrations (each one is a manual step in the Slack app configuration and, for a public repo, a setup instruction someone else must follow), the `/jt` namespace whose verb table is the single source, and every verb in three groups — ready, blocked on a deterministic implementation, blocked on durable state. Carries the channel-as-routing-accident defect (a verb typed in an agent channel is answered by a model as conversation, bridge-agent.js:1298) and the two operations that are NOT commands because the container has no route to the host's container runtime. It describes INTENT; CLAUDE.md -> Built-in Commands describes what is registered
