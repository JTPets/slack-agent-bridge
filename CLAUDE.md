...
│   ├── dispatch-command.js # THE COMMAND AND FORM halves of /dispatch: buildModalView builds the five-input modal FROM lib/dispatch-message.js's FIELD_KEYS (so neither side can gain a field alone), extractSubmission reads the five values back out of a view_submission, toSlackErrors keys a rejection by block_id so it lands on the offending input, and handleSlashCommand/handleViewSubmission attach at THE COMMAND SEAM in lib/slack-socket.js. Authorisation is this module's own gate via lib/config.js isUserAuthorized — a bot-posted message bypasses the poll loop's allowlist by design, so there is no second gate downstream
...
│   ├── dispatch-command.test.js # Tests for lib/dispatch-command.js: the five-input modal shape, extraction of a blank optional input (Slack sends null), block_id-keyed errors, the three-second ack ordering, the authorisation gate, and every failure path (modal open failure, post failure, post timeout)
